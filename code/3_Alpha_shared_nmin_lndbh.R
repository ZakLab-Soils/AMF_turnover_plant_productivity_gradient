...
chao.scatter.data.shared <-  data.chao.ps.amf.F.prune.shared.rare %>%
  select(sampleID, Nmin_rate, lndbh, value, se)
...
##Richness scatterplot regression (Chao1) by tree growth/size (lnDBH) - NOTE there are 2 DBH measurments missing from the data

scatter.chao.lndbh <- ggplot(data=chao.scatter.data.shared, aes(x=lndbh, y=value)) + geom_point(color = "#454545") + geom_smooth(method="lm", color = "#458B00") + xlab(paste0("lnDBH")) + ylab(paste0("Chao1 Richness ")) + ggtitle("Chao1 species richness along tree size (lnDBH)") + theme_bw() + annotate(geom="text", x=2.6,y=c(90,85),size=4, label=c(r2.pvalue.label$labelr2[2], r2.pvalue.label$labelpv[2]), parse=TRUE)
