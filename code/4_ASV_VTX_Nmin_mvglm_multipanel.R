...
psmelt.simple_asv %>% 
  ...
  xlab("Net N mineralization rate (µg N"~g^-1~d^-1~")") + ylab(paste0("Relative Abundance (%)")) + 
...
#Create a figure with muliple panels

Figure_ASV_VT <- ggarrange(ASV_plot +
                        labs(x="", y="", title=""), 
                      VT_plot + 
                        labs(x="", y="", title=""),
                      nrow=2, 
...
#xlab("Net N mineralization rate (µg N"~g^-1~d^-1~")")

annotate_figure(Figure_ASV_VT, left = "Relative Abundance (%)", bottom = text_grob(bquote("Net N mineralization rate (µg N"~g^-1~d^-1~")"))
)

ggsave("Relative_Abundance_Nmin_Multipanel.pdf", height = 11, width=8.5)
